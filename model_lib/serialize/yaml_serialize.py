...
def dump_yaml_str(
    ...
    try:
        ...
        if isinstance(maybe_base_model, BaseModel):
            try:
                register_dumper(type(maybe_base_model), base_model_dumper, allow_override="never")
            except DumperExist:
                raise e from None
...
def dump_yaml_file(path: PathLike | str, data: Mapping, width=1000, sort_keys: bool = False):
    with open(path, "w") as f:
        yaml.safe_dump(data, stream=f, default_flow_style=False, width=width, sort_keys=sort_keys)
    return path
...
class edit_yaml:
    ...
    def __enter__(self):
        with open(self.path) as f:
            ...
            if self.yaml_path:
                self.to_update = read_nested(self.loaded, self.yaml_path.replace(" ", "."))
...
_start_bracket_re = re.compile(rf"'?{edit_helm_template.START_BRACKETS_REPLACEMENT[1:]}")
...
class no_timestamp_conversion:
    ...
    def __enter__(self):
        ...
        self.old_timestamp_constructor = _NoTimestampSafeLoader.yaml_constructors.get(timestamp_tag)
...
def _make_yaml_line(line):
    ...
    return line.replace('"', quote_replacement).replace("{{", start_replacement).replace("}}", end_replacement)
...
def _indent_standalone_templates(raw: str) -> str:
    ...
    for i, line in enumerate(raw.splitlines(keepends=False)):
        ...
        elif "START_BRACKETS" in line and ended_with_colon and line_indentation_level == indentation_level:
